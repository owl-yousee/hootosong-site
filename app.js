...
function confirmCommitSetlist(){
  ...
  const message=hasExisting?"今日の日付には既存の歌枠アーカイブがあります。\n現在の仕様では、同じ日付のアーカイブを更新します。\n曲の歌唱回数・最終歌唱日はここでは更新しません。\nよろしいですか？":"今日のセトリを歌枠アーカイブに保存します。\n曲の歌唱回数・最終歌唱日はここでは更新しません。\nよろしいですか？";
...
function deleteStreamArchive(index){const stream=streams[index];if(!stream)return;const name=stream.name||stream.streamTitle||"名称未設定",date=normalizeDateString(stream.date)||stream.date||"";if(!confirm(`「${name}」 ${date} の歌枠アーカイブを削除します。\n曲の歌唱回数・最終歌唱日は変更されません。\nよろしいですか？`))return;streams.splice(index,1);saveStreams();renderHistory();render();showToast("歌枠アーカイブを削除しました");}
...
$("#createStream").addEventListener("click",()=>{const date=els.streamDate.value;if(!date){showToast("日付を選んでください");return;}if(streams.some(stream=>stream.date===date)){showToast("この日の歌枠はすでにあります");return;}streams.push(normalizeStream({date,songs:[]}));saveStreams();renderHistory();showToast("歌枠を作成しました");});
